...
class BoardgamedesignerSpider(scrapy.Spider):
    name = "boardgamedesigner"
    allowed_domains = ["api.geekdo.com"]
    start_urls = ["https://api.geekdo.com/xmlapi/"]

    def __init__(self, *args, **kwargs):
        super(BoardgamedesignerSpider, self).__init__(*args, **kwargs)
    ...
    custom_settings = {
        'FEEDS': {
            os.path.join('scrapped_data', 'bgg_designer_data.json'): {
                'format': 'json',
                'encoding': 'utf-8',
                'ensure_ascii': False,
                'indent': 4,
    ...
    def start_requests(self):
        filename = './data/designers_objectids.json'
    ...
    def parse(self, response):
        ...
        boardgame_designer = self.build_boardgamepublisher(root, id, final_url, boardgames)
        yield boardgame_designer.dict()
    ...
    def build_boardgamepublisher(self, root, id, url, boardgames):
        ...
        author = 22205250
        id = int(id)
        company = Designer(author, id, url, name, description, boardgames)
        return company
